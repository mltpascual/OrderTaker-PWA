  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-700">
      <div className="flex justify-between items-end">
        ...
        <button 
          ...
          className={`px-6 py-2.5 rounded-xl font-bold text-sm transition-all flex items-center gap-2 ${
            isAdding ? 'bg-rose-50 text-rose-600' : 'bg-indigo-600 text-white shadow-lg shadow-indigo-100 hover:scale-105'
          }`}
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {menu.map((cake) => (
          <div key={cake.id} className="bg-white p-6 rounded-[1.5rem] border border-slate-100 shadow-md shadow-slate-200/40 hover:shadow-lg transition-all group flex flex-col min-h-[160px]">
            <div className="flex justify-between items-start mb-4">
              <div className="w-12 h-12 bg-indigo-50/80 rounded-xl flex items-center justify-center text-indigo-600 font-black text-xl shadow-inner">
                {cake.name.charAt(0)}
              </div>
            ...
            <div className="flex-1 mb-4">
              <h3 className="font-black text-slate-900 leading-tight text-lg">{cake.name}</h3>
            </div>
            
            <div className="flex items-end justify-between pt-4 border-t border-slate-50">
              <span className="text-2xl font-black text-indigo-600 tracking-tight">${cake.basePrice.toFixed(2)}</span>
              <span className="text-[9px] font-black text-slate-300 uppercase tracking-[0.15em] mb-1.5">Base Price</span>
            </div>
          </div>
        ))}
      </div>
